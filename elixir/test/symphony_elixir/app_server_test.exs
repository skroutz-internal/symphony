defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server advertises tracker-provided dynamic tools only" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-dynamic-tools-by-tracker-#{System.unique_integer([:positive])}"
      )

    try do
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "MT-90T")
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-dynamic-tools-by-tracker.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-dynamic-tools-by-tracker.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
        ...
        case \"$count\" in
          ...
          2)
            ;;
          3)
            printf '%s\\n' '{"id":2,"result":{"thread":{"id":"thread-90t"}}}'
            ;;
          4)
            printf '%s\\n' '{"id":3,"result":{"turn":{"id":"turn-90t"}}}'
            printf '%s\\n' '{"method":"turn/completed"}'
            exit 0
            ;;
          *)
      ...
      issue = %Issue{
        id: "issue-tool-advertisement",
        identifier: "MT-90T",
        title: "Tracker-specific dynamic tools",
        description: "Ensure only tracker-provided dynamic tools are advertised",
        state: "In Progress",
        url: "https://example.org/issues/MT-90T",
        labels: ["backend"]
      }

      assert {:ok, _result} = AppServer.run(workspace, "Advertise tracker tools", issue)
  ...
  test "app server advertises github dynamic tools for github tracker" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-github-tools-#{System.unique_integer([:positive])}"
      )

    try do
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "MT-90GH")
      codex_binary = Path.join(test_root, "fake-codex")
      trace_file = Path.join(test_root, "codex-github-tools.trace")
      previous_trace = System.get_env("SYMP_TEST_CODEx_TRACE")
      ...
      File.write!(codex_binary, """
      #!/bin/sh
      trace_file="${SYMP_TEST_CODEx_TRACE:-/tmp/codex-github-tools.trace}"
      count=0
      while IFS= read -r line; do
        count=$((count + 1))
        printf 'JSON:%s\\n' \"$line\" >> \"$trace_file\"
        ...
        case \"$count\" in
          ...
          2)
            ;;
          3)
            printf '%s\\n' '{"id":2,"result":{"thread":{"id":"thread-90gh"}}}'
            ;;
          4)
            printf '%s\\n' '{"id":3,"result":{"turn":{"id":"turn-90gh"}}}'
            printf '%s\\n' '{"method":"turn/completed"}'
            exit 0
            ;;
          *)
      ...
      issue = %Issue{
        ...
        description: "Ensure GitHub tracker advertises GitHub dynamic tools",
        state: "Todo",
        url: "https://github.com/owner/repo/issues/1",
        labels: ["backend"]
      }

      assert {:ok, _result} = AppServer.run(workspace, "Advertise github tracker tools", issue)
                   ...
                   tools when is_list(tools) ->
                     Enum.any?(tools, fn
                       %{
                         ...
                         payload["id"] == 2 and description =~ "GitHub"
  ...
  test "app server via shim lets pi push registered tools back to symphony" do
    ...
    try do
      ...
      assert {:ok, _result} =
               AppServer.run(workspace, "/print-tools", issue, on_message: fn message -> send(test_pid, {:app_server_message, message}) end)
  ...
  test "app server via shim lets pi push loaded skills back to symphony" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-shim-push-skills-#{System.unique_integer([:positive])}"
      )

    try do
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "MT-SHIM-SKILLS")
      repo_root = Path.expand("../../..", __DIR__)
      test_pid = self()
      ...
      write_workflow_file!(Workflow.workflow_file_path(),
        ...
        workspace_root: workspace_root,
        codex_command: "cd #{repo_root} && PI_BIN=#{repo_root}/pi-with-pts node ./pi-codex-shim.ts"
      )

      issue = %Issue{
        id: "owner/repo#1:PVTI_1",
        identifier: "owner/repo#1",
        title: "Shim skill visibility",
        description: "Ensure pi can push its loaded skills back to Symphony through the shim",
        state: "Todo",
        url: "https://github.com/owner/repo/issues/1",
        labels: ["backend"]
      }
      ...
      assert "skill:land" in skill_names
    after
      File.rm_rf(test_root)
    end
  end
